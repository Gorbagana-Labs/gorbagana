use {
    crate::vote_transaction::VoteTransaction, gorbagana_bincode::limited_deserialize,
    gorbagana_hash::Hash, gorbagana_pubkey::Pubkey, gorbagana_signature::Signature,
    gorbagana_svm_transaction::svm_transaction::SVMTransaction, gorbagana_transaction::Transaction,
    gorbagana_vote_interface::instruction::VoteInstruction,
};
...
pub fn parse_sanitized_vote_transaction(tx: &impl SVMTransaction) -> Option<ParsedVote> {
    ...
    if !gorbagana_sdk_ids::vote::check_id(program_id) {
        return None;
    }
...
mod test {
    use {
        ...
        gorbagana_vote_interface::{instruction as vote_instruction, state::Vote},
    };
