...
## Run gorbagana validator-info

Run the gorbagana CLI to populate a validator info account:

```bash
gorbagana validator-info publish --keypair ~/validator-keypair.json <VALIDATOR_INFO_ARGS> <VALIDATOR_NAME>
```

For details about optional fields for VALIDATOR_INFO_ARGS:

```bash
gorbagana validator-info publish --help
```
...
```bash
gorbagana validator-info publish "Elvis Validator" -w "https://elvis-validates.com" -i "https://elvis-validates.com/my-icon.png"
```

Example query command:

```bash
gorbagana validator-info get
```
