---
title: Send and Receive Tokens with the Gorbagana CLI
pagination_label: "Gorbagana CLI: Send and Receive Tokens"
...
```bash
gorbagana airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://api.devnet.gorbagana.com
```
...
```bash
gorbagana confirm -v <TRANSACTION_SIGNATURE>
```
...
```bash
gorbagana balance <ACCOUNT_ADDRESS> --url https://api.devnet.gorbagana.com
```
...
```bash
gorbagana-keygen new --no-passphrase --no-outfile
```
...
The Gorbagana cluster will only accept the transfer if you sign the transaction
with the private keypair corresponding to the sender's public key in the
transaction.

```bash
gorbagana transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://api.devnet.gorbagana.com --fee-payer <KEYPAIR>
```
...
Confirm the updated balances with `gorbagana balance`:

```bash
gorbagana balance <ACCOUNT_ADDRESS> --url http://api.devnet.gorbagana.com
```
...
```bash
$ gorbagana-keygen new --outfile my_gorbagana_wallet.json   # Creating my first wallet, a file system wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
Wrote new keypair to my_gorbagana_wallet.json
==========================================================================
...
$ gorbagana airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.gorbagana.com  # Airdropping 1 SOL to my wallet's address/pubkey
Requesting airdrop of 1 SOL from 35.233.193.70:9900
1 SOL

$ gorbagana balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.gorbagana.com # Check the address's balance
1 SOL

$ gorbagana-keygen new --no-outfile  # Creating a second wallet, a paper wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
====================================================================
...
$ gorbagana transfer --from my_gorbagana_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://api.devnet.gorbagana.com --fee-payer my_gorbagana_wallet.json  # Transferring tokens to the public address of the paper wallet
3gmXvykAd1nCQQ7MjosaHLf69Xyaqyq1qw2eu1mgPyYXd5G4v1rihhg1CiRw35b9fHzcftGKKEu4mbUeXY2pEX2z  # This is the transaction signature

$ gorbagana balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.gorbagana.com
...
$ gorbagana balance 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv --url https://api.devnet.gorbagana.com
...
Gorbagana, the wallet address is the public key of a keypair. There are a variety
...
with the `gorbagana transfer` command:

```bash
gorbagana transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> <AMOUNT> --fee-payer <KEYPAIR>
```

Confirm the updated balances with `gorbagana balance`:

```bash
gorbagana balance <ACCOUNT_ADDRESS>
```
