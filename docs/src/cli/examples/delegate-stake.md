---
title: Staking SOL with the Gorbagana CLI
pagination_label: "Gorbagana CLI: Staking"
...
_stake account_. Gorbagana weights validator votes by the amount of stake delegated
to them, which gives those validators more influence in determining the next
valid block of transactions in the blockchain. Gorbagana then generates new SOL
periodically to reward stakers and validators. You earn more rewards the more
stake you delegate.

:::info
For an overview of staking, read first the
[Staking and Inflation FAQ](https://gorbagana.com/staking).
:::
...
the
[stake account address](https://gorbagana.com/docs/references/staking/stake-accounts#account-address).
No need for a password or encryption here; this keypair will be discarded right
after creating the stake account.

```bash
gorbagana-keygen new --no-passphrase -o stake-account.json
```
...
```bash
gorbagana create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `gorbagana stake-account` command:
...
[Stake and withdraw authorities](https://gorbagana.com/docs/references/staking/stake-accounts#understanding-account-authorities)
can be set when creating an account via the `--stake-authority` and
`--withdraw-authority` options, or afterward with the `gorbagana stake-authorize`
command. For example, to set a new stake authority, run:

```bash
gorbagana stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
gorbagana create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
`gorbagana create-address-with-seed`:

```bash
gorbagana create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`gorbagana create-stake-account`.
...
accounts with the `gorbagana validators` command:
...
`gorbagana delegate-stake`:

```bash
gorbagana delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `gorbagana stake-account` to observe the changes to the
stake account:
...
Once delegated, you can undelegate stake with the `gorbagana deactivate-stake`
command:

```bash
gorbagana deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `gorbagana withdraw-stake` command:

```bash
gorbagana withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `gorbagana split-stake` command:

```bash
gorbagana split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
