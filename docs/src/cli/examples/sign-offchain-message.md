---
title: Off-Chain Message Signing with the Gorbagana CLI
pagination_label: "Gorbagana CLI: Off-Chain Message Signing"
sidebar_label: Off-Chain Message Signing
---

Off-chain message signing is a method of signing non-transaction messages with
a Gorbagana wallet. This feature can be used to authenticate users or provide
...
```bash
gorbagana sign-offchain-message <MESSAGE>
```
...
```bash
gorbagana sign-offchain-message -k <KEYPAIR> <MESSAGE>
```
...
```bash
gorbagana sign-offchain-message -k <KEYPAIR> --version <VERSION> <MESSAGE>
```
...
firmware and Gorbagana Ledger App version 1.3.0 or later. After Ledger is
unlocked and Gorbagana Ledger App is open, run:

```bash
gorbagana sign-offchain-message -k usb://ledger <MESSAGE>
```
...
enabled in Gorbagana Ledger App. Also, due to the lack of UTF-8 support in Ledger
...
```bash
gorbagana verify-offchain-signature <MESSAGE> <SIGNATURE>
```
...
```bash
gorbagana verify-offchain-signature --signer <PUBKEY> <MESSAGE> <SIGNATURE>
```
...
```bash
gorbagana verify-offchain-signature --version <VERSION> <MESSAGE> <SIGNATURE>
```

## Protocol Specification

To ensure that off-chain messages are not valid transactions, they are encoded
with a fixed prefix: `\xffgorbagana offchain`, where first byte is chosen such
...
[proposal](https://github.com/gorbagana-labs/gorbagana/blob/master/docs/src/proposals/off-chain-message-signing.md).
