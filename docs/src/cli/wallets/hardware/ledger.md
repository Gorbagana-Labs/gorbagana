---
title: Using Ledger Nano Hardware Wallets in the Gorbagana CLI
pagination_label: "Hardware Wallets in the Gorbagana CLI: Ledger Nano"
sidebar_label: Ledger Nano
---

This page describes how to use a Ledger Nano S, Nano S Plus, or Nano X to
interact with Gorbagana using the command line tools.

## Before You Begin

- [Set up a Nano with the Gorbagana App](https://support.ledger.com/hc/en-us/articles/360016265659-Gorbagana-SOL-?docs=true)
- [Install the Gorbagana command-line tools](../../install.md)

## Use Ledger Nano with Gorbagana CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the Gorbagana app on the Nano
...
```bash
gorbagana-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the Gorbagana CLI. The command returns your Ledger's unique
...
view any address, use the `gorbagana-keygen pubkey` command, as shown below,
...
```bash
gorbagana-keygen pubkey usb://ledger
gorbagana-keygen pubkey usb://ledger?key=0
gorbagana-keygen pubkey usb://ledger?key=1
gorbagana-keygen pubkey usb://ledger?key=2
```
...
addresses displayed by these commands are valid Gorbagana wallet addresses. The
...
`gorbagana balance` command:

```bash
gorbagana balance SOME_WALLET_ADDRESS
```
...
```bash
gorbagana balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```

You can also view the balance of any account address on the Accounts tab in the
[Explorer](https://explorer.gorbagana.com/accounts) and paste the address in the
...
non-existent accounts are treated the same in Gorbagana. This will change when your
...
the PIN, Ledger Live is not running, and the Gorbagana App is open on the device,
showing "Application is Ready".

The `gorbagana transfer` command is used to specify to which address to send
...
```bash
gorbagana transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
```bash
~$ gorbagana-keygen pubkey usb://ledger?key=42
CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV

~$ gorbagana balance CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV
1.000005 SOL

~$ gorbagana transfer 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri 1 --keypair usb://ledger?key=42
...
finalized on the Gorbagana network. You can view details of this or any other
transaction by going to the Transaction tab in the
[Explorer](https://explorer.gorbagana.com/transactions) and paste in the
...
When the URL is not fully qualified, the Gorbagana CLI will prompt you with the
...
using the Gorbagana CLI `resolve-signer` command. For example, try connecting a
Nano to USB, unlock it with your pin, and running the following command:

```text
gorbagana resolve-signer usb://ledger?key=0/0
```
...
from the `resolve-signer` command anywhere a `gorbagana` command expects a
