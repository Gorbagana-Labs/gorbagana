#!/usr/bin/env bash
#
# gorbagana-cli integration sanity test
#
set -e
...
while [[ $timeout -gt 0 ]]; do
  set +e
  output=$($gorbagana_cli "${args[@]}" transaction-count --commitment finalized)
  rc=$?
  set -e
...
(
  set -x
  $gorbagana_cli "${args[@]}" address
  $gorbagana_cli "${args[@]}" balance
  $gorbagana_cli "${args[@]}" ping --count 5 --interval 0
  $gorbagana_cli "${args[@]}" balance
)
