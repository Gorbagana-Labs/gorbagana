...
case $URL in
stable)
  URL=http://api.devnet.gorbagana.com
  ;;
localhost)
...
case $TAG in
edge|beta)
  DOWNLOAD_URL=https://release.anza.xyz/"$TAG"/gorbagana-release-$TARGET.tar.bz2
  ;;
*)
  DOWNLOAD_URL=https://github.com/anza-xyz/agave/releases/download/"$TAG"/gorbagana-release-$TARGET.tar.bz2
  ;;
esac
...
balance=$(gorbagana $maybeKeypair --url "$URL" balance --lamports)
if [[ $balance = "0 lamports" ]]; then
  # shellcheck disable=SC2086 # Don't want to double quote $maybeKeypair
  gorbagana $maybeKeypair --url "$URL" airdrop 0.000000042
fi
