use {
    ...
    gorbagana_program_test::{ProgramTest, ProgramTestContext},
...
async fn test_vended_core_bpf_programs() {
    ...
    assert_bpf_program(&context, &gorbagana_sdk_ids::address_lookup_table::id()).await;
    assert_bpf_program(&context, &gorbagana_sdk_ids::config::id()).await;
    assert_bpf_program(&context, &gorbagana_sdk_ids::feature::id()).await;
}

#[tokio::test]
async fn test_add_core_bpf_program_manually() {
    // Core BPF program: Stake.
    let program_id = gorbagana_sdk_ids::stake::id();
