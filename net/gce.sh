...
if [[ $cloudProvider = ec2 ]]; then
  ...
  sshPrivateKey="$HOME/.ssh/gorbagana-net-id_$prefix"
else
    ...
    if [[ ! -r $sshPrivateKey ]]; then
      ...
      (
        ...
        for i in $(seq 1 60); do
          set -x
          cloud_FetchFile "$nodeName" "$nodeIp" /gorbagana-scratch/id_ecdsa "$sshPrivateKey" "$nodeZone" &&
            cloud_FetchFile "$nodeName" "$nodeIp" /gorbagana-scratch/id_ecdsa.pub "$sshPrivateKey.pub" "$nodeZone" &&
              break
          set +x
  ...
  recordInstanceIp() {
    ...
    (
      ...
      for i in $(seq 1 60); do
        (
          set -x
          timeout --preserve-status --foreground 20s ssh "${sshOptions[@]}" "$publicIp" "ls -l /gorbagana-scratch/.instance-startup-complete"
        )
        ret=$?
        if [[ $ret -eq 0 ]]; then
...
if [[ -f /gorbagana-scratch/.instance-startup-complete ]]; then
  echo reboot
  $(
    cd "$here"/scripts/
    if "$enableGpu"; then
  ...
  if [[ -x ~gorbagana/gorbagana/on-reboot ]]; then
    sudo -u gorbagana ~gorbagana/gorbagana/on-reboot
  fi
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  To block until setup is complete, run:
    $ until [[ -f /gorbagana-scratch/.instance-startup-complete ]]; do sleep 1; done

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$(creationInfo)
EOM

# Place the generated private key at /gorbagana-scratch/id_ecdsa so it's retrievable by anybody
# who is able to log into this machine
mkdir -m 0777 /gorbagana-scratch
cat > /gorbagana-scratch/id_ecdsa <<EOK
$(cat "$sshPrivateKey")
EOK
cat > /gorbagana-scratch/id_ecdsa.pub <<EOK
$(cat "$sshPrivateKey.pub")
EOK
chmod 444 /gorbagana-scratch/id_ecdsa
...
cat >/gorbagana-scratch/gce-self-destruct.sh <<'EOS'
...
cat >/gorbagana-scratch/gce-self-destruct-ps1.sh <<'EOS'
#!/usr/bin/env bash
source "$(dirname "$0")/gce-self-destruct.sh"
gce_self_destruct_ps1
EOS
chmod +x /gorbagana-scratch/gce-self-destruct-ps1.sh

# Append MOTD and PS1 replacement to .profile
cat >>~gorbagana/.profile <<'EOS'

# Print self-destruct countdown on login
source "/gorbagana-scratch/gce-self-destruct.sh"
gce_self_destruct_motd

# Add self-destruct countdown to terminal prompt
export PS1='\[\e]0;\u@\h: \w\a\]${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]$(/gorbagana-scratch/gce-self-destruct-ps1.sh):\[\033[01;34m\]\w\[\033[00m\]\$ '
EOS
EOSD
    cat <<EOSD
...
tmpfsMountPoint=/mnt/gorbagana-accounts
...
touch /gorbagana-scratch/.instance-startup-complete
