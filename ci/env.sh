...
if [[ -n $CI ]]; then
  ...
  if [[ -n $BUILDKITE ]]; then
    ...
    # to how gorbagana-ci-gate is used to trigger PR builds rather than using the
    ...
    *)
    ...
    if [[ -n $BUILDKITE_TRIGGERED_FROM_BUILD_PIPELINE_SLUG ]]; then
      # The gorbagana-secondary pipeline should use the slug of the pipeline that
      # triggered it
    ...
    # the gorbagana-secondary pipeline
