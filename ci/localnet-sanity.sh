...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $gorbagana_cli --keypair config/bootstrap-validator/identity.json \
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $gorbagana_ledger_tool --ledger "$SOLANA_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $gorbagana_keygen new --no-passphrase -fso $client_keypair || exit $?
    $gorbagana_gossip --allow-private-addr spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
